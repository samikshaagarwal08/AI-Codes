...
# def create_board():
#     return [[None if (r + c) % 2 == 0 else ('b' if r < 3 else 'w') for c in range(COLS)] for r in range(ROWS)]
...
#     if dx == dy == 2:
#         mid = ((start[0] + end[0]) // 2, (start[1] + end[1]) // 2)
#         if board[mid[1]][mid[0]] not in [None, player]: return True
#     return False

# def ai_move(board, player):
#     valid_moves = [(start, end) for r in range(ROWS) for c in range(COLS) if board[r][c] == player
#                    for dr, dc in [(-2, -2), (-2, 2), (2, -2), (2, 2)] if 0 <= (end := (c + dc, r + dr))[0] < COLS and 0 <= end[1] < ROWS and is_valid_move(board, (c, r), end, player)]
#     return random.choice(valid_moves) if valid_moves else None

# def main():
#     board, player_turn = create_board(), 'w'
#     while True:
#         screen.fill((0, 0, 0))
#         draw_board(board)
#         for event in pygame.event.get():
#             if event.type == pygame.QUIT: pygame.quit(); sys.exit()
#             if event.type == pygame.MOUSEBUTTONDOWN and player_turn == 'w':
#                 col, row = event.pos[0] // SIZE, event.pos[1] // SIZE
#                 if board[row][col] == 'w':
#                     move = ai_move(board, 'b')
#                     if move: 
#                         (start_col, start_row), (end_col, end_row) = move
